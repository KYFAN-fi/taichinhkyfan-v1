...
const DEFAULT_CUSTOMERS = [
  { id: 1, name: "Nguyễn Văn An", phone: "0901 234 567", principal: 25000000, interest: 1000000, paid: 0, status: "overdue", note: "Vay lấy vốn kinh doanh", color: "#49b15b" },
  { id: 2, name: "Trần Thị Bình", phone: "0912 345 678", principal: 15000000, interest: 800000, paid: 0, status: "due", note: "Ưu tiên liên hệ qua Zalo", color: "#f4a913" },
  { id: 3, name: "Lê Minh Cường", phone: "0933 466 789", principal: 12000000, interest: 600000, paid: 0, status: "reminded", note: "Đã gửi tin nhắn nhắc nợ", color: "#48a6e8" },
  { id: 4, name: "Phạm Thùy Linh", phone: "0977 888 999", principal: 8000000, interest: 400000, paid: 8400000, status: "paid", note: "Đã hoàn tất kỳ thanh toán", color: "#40aa53" },
  { id: 5, name: "Hoàng Văn Dũng", phone: "0988 777 666", principal: 20000000, interest: 1000000, paid: 0, status: "paused", note: "Tạm hoãn theo thỏa thuận", color: "#858b94" },
  { id: 6, name: "Võ Ngọc Mai", phone: "0965 222 140", principal: 18500000, interest: 900000, paid: 0, status: "unreminded", note: "Liên hệ sau 17 giờ", color: "#a86ed0" }
];

const DEFAULT_NOTIFICATIONS = [
  { id: 1, type: "overdue", title: "Khoản vay quá hạn", message: "Nguyễn Văn An chưa hoàn tất thanh toán kỳ này.", time: "08:30", read: false },
  { id: 2, type: "reminded", title: "Đã gửi nhắc nợ", message: "Tin nhắn nhắc nợ đã được tạo cho Lê Minh Cường.", time: "09:15", read: false },
  { id: 3, type: "paid", title: "Thanh toán thành công", message: "Phạm Thùy Linh đã thanh toán 8.400.000 đ.", time: "10:20", read: true },
  { id: 4, type: "paused", title: "Tạm hoãn thanh toán", message: "Hoàng Văn Dũng được chuyển sang trạng thái tạm hoãn.", time: "11:05", read: true }
];

const DEFAULT_SETTINGS = { early: true, repeat: true, browser: true, zalo: true, call: true };
const MONTHS = ["Tháng 1", "Tháng 2", "Tháng 3", "Tháng 4", "Tháng 5", "Tháng 6", "Tháng 7", "Tháng 8", "Tháng 9", "Tháng 10", "Tháng 11", "Tháng 12"];
const WEEKDAYS = ["T2", "T3", "T4", "T5", "T6", "T7", "CN"];

let customers = loadData(STORAGE_KEYS.customers, DEFAULT_CUSTOMERS).map(customer => ({
  ...customer,
...
function formatShortMoney(value) {
  ...
  if (value >= 1_000_000) return `${new Intl.NumberFormat("vi-VN").format(value / 1_000_000)} tr`;
...
function getTotals() {
  const total = customers.reduce((sum, customer) => sum + customer.principal + customer.interest, 0);
  const paid = customers.reduce((sum, customer) => sum + customer.paid, 0);
  const remaining = Math.max(0, total - paid);
  const due = customers.filter(customer => ["unreminded", "due", "overdue", "reminded"].includes(customer.status)).length;
  const unreminded = customers.filter(customer => customer.status === "unreminded").length;
  const overdue = customers.filter(customer => customer.status === "overdue").length;
  const reminded = customers.filter(customer => customer.status === "reminded").length;
  const paidCount = customers.filter(customer => customer.status === "paid").length;
  return { total, paid, remaining, due, unreminded, overdue, reminded, paidCount };
}
...
function statusSelect(customer, context = "card") {
  ...
  const options = Object.entries(STATUS).map(([value, status]) =>
...
function setActiveView(viewName) {
  document.querySelectorAll(".app-view").forEach(view => view.classList.toggle("active", view.dataset.view === viewName));
  ...
  document.getElementById("mobile-page-title").textContent = titles[viewName].replace(" hôm nay", "");
...
function buildCalendar(date, full = false) {
  ...
  for (let i = 0; i < cellCount; i += 1) {
    ...
    const classes = ["calendar-day", isOther ? "other-month" : "", isToday ? "today" : "", isReminder ? "reminder-day" : ""].filter(Boolean).join(" ");
...
function renderDateInformation() {
  ...
  document.getElementById("hero-next-month").textContent = `${MONTHS[next.getMonth()]} / ${next.getFullYear()}`;
  document.getElementById("hero-countdown").textContent = diff === 0 ? "Nhắc trong hôm nay" : `Còn ${diff} ngày`;
  document.getElementById("preview-due-date").textContent = dateText(dueDateFor(next));
  document.getElementById("report-month").textContent = `${MONTHS[today.getMonth()]} / ${today.getFullYear()}`;
}

function renderDashboard() {
  const totals = getTotals();
  const stats = [
    { label: "Tổng công nợ", value: formatShortMoney(totals.total), note: `${customers.length} khách hàng`, icon: "wallet", tone: "blue" },
    { label: "Cần thu kỳ này", value: totals.due, note: "Khách đang chờ xử lý", icon: "users", tone: "orange" },
    { label: "Đã nhắc", value: totals.reminded, note: "Đã tạo thông báo", icon: "send", tone: "blue" },
    { label: "Đã thanh toán", value: totals.paidCount, note: formatShortMoney(totals.paid), icon: "check", tone: "green" }
  ];
  ...
  const today = new Date();
  document.getElementById("mini-calendar-title").textContent = `${MONTHS[today.getMonth()]} / ${today.getFullYear()}`;
  document.getElementById("mini-calendar").innerHTML = buildCalendar(today);

  const priority = customers.filter(customer => customer.status !== "paid").sort((a, b) => {
    const order = { overdue: 0, due: 1, unreminded: 2, reminded: 3, paused: 4 };
    return (order[a.status] ?? 5) - (order[b.status] ?? 5);
  }).slice(0, 5);

  document.getElementById("dashboard-customer-list").innerHTML = priority.length ? priority.map(customer => `
  ...
  `).join("") : emptyState("Không có khách cần xử lý", "Các khoản nợ trong kỳ đã hoàn tất.");
}

function renderCustomers() {
  const query = document.getElementById("customer-search").value.trim().toLocaleLowerCase("vi");
  const filtered = customers.filter(customer => {
    const matchesQuery = [customer.name, customer.phone, customer.note].join(" ").toLocaleLowerCase("vi").includes(query);
    const matchesFilter = activeCustomerFilter === "all" || customer.status === activeCustomerFilter;
    return matchesQuery && matchesFilter;
  }).sort((a, b) => {
    const order = { overdue: 0, due: 1, unreminded: 2, reminded: 3, paused: 4, paid: 5 };
    return (order[a.status] ?? 6) - (order[b.status] ?? 6);
  });

  document.getElementById("customer-result-count").textContent = `${filtered.length} khách hàng`;
  document.getElementById("customer-grid").innerHTML = filtered.length ? filtered.map(customer => `
  ...
  `).join("") : emptyState("Không tìm thấy khách hàng", "Thử thay đổi từ khóa hoặc bộ lọc.");
}
...
function renderCalendarPage() {
  const totals = getTotals();
  document.getElementById("full-calendar-title").textContent = `${MONTHS[calendarCursor.getMonth()]} / ${calendarCursor.getFullYear()}`;
  document.getElementById("full-calendar").innerHTML = buildCalendar(calendarCursor, true);
  ...
  ].map(([label, value, tone]) => `<div class="calendar-mini-stat ${tone}"><span>${label}</span><strong>${value}</strong></div>`).join("");

  const year = calendarCursor.getFullYear();
  const month = calendarCursor.getMonth();
  const schedule = [
    { day: 7, title: "Nhắc trước hạn", info: "Chuẩn bị trước 3 ngày", count: totals.due },
    { day: 10, title: "Nhắc đến hạn", info: "Ngày gửi chính thức", count: totals.due },
    { day: 13, title: "Nhắc lại lần 1", info: "Nếu chưa thanh toán", count: totals.overdue }
  ];
  document.getElementById("reminder-schedule").innerHTML = schedule.map(item => {
...
function renderNotifications() {
  ...
  }).join("") : emptyState("Chưa có thông báo", "Hoạt động nhắc nợ sẽ xuất hiện tại đây.");
...
function updateNotificationBadges() {
  const unread = notifications.filter(item => !item.read).length;
  ["top-notification-count", "mobile-notification-count", "sidebar-notification-count"].forEach(id => {
    const badge = document.getElementById(id);
    badge.textContent = unread;
    badge.classList.toggle("visible", unread > 0);
  });
  document.getElementById("bottom-notification-dot").classList.toggle("visible", unread > 0);
}

function renderReport() {
  const totals = getTotals();
  const rate = totals.total ? Math.round((totals.paid / totals.total) * 100) : 0;
  document.getElementById("recovery-ring").style.setProperty("--rate", `${rate}%`);
  document.getElementById("recovery-rate").textContent = `${rate}%`;
  document.getElementById("report-total").textContent = formatMoney(totals.total);
  document.getElementById("report-paid").textContent = formatMoney(totals.paid);
  document.getElementById("report-remaining").textContent = formatMoney(totals.remaining);

  const now = new Date();
  const revenue = [42, 58, 51, 73, 68, Math.max(8, Math.round(totals.paid / 1_000_000))];
  const max = Math.max(...revenue, 1);
  document.getElementById("revenue-chart").innerHTML = revenue.map((value, index) => {
...
function openCustomerDetail(customerId) {
  ...
  if (!customer) return;
  const remaining = Math.max(0, customer.principal + customer.interest - customer.paid);
...
function sendReminder(customerId = null, automatic = false) {
  const targets = customerId
    ...
    : customers.filter(customer => !["paid", "paused"].includes(customer.status));

  if (!targets.length) {
    showToast("Không có khách cần nhắc", "Tất cả khách hàng đã thanh toán hoặc đang tạm hoãn.", "info");
    return;
  }
  ...
  renderAll();
  showToast("Tạo nhắc nợ thành công", `Đã cập nhật ${targets.length} khách hàng.`, "success");

  if (settings.browser && "Notification" in window && Notification.permission === "granted") {
    new Notification("M10 · Nhắc thu nợ", {
      body: customerId ? `Đến lịch nhắc ${targets[0].name}.` : `${targets.length} khách hàng cần được nhắc thanh toán.`,
...
function updateCustomerStatus(customerId, newStatus, keepDetailOpen = false) {
  ...
  notifications.unshift({
    id: Date.now(),
    type: newStatus,
    title: newStatus === "paid" ? "Thanh toán thành công" : "Đã cập nhật trạng thái",
...
async function requestBrowserNotification() {
  if (!("Notification" in window)) {
    showToast("Thiết bị chưa hỗ trợ", "Trình duyệt này không cung cấp Notification API.", "info");
    return;
  }
  ...
  if (permission === "granted") {
    ...
    showToast("Đã bật thông báo", "M10 có thể hiển thị nhắc nợ khi trang đang mở.", "success");
  } else {
    showToast("Chưa được cho phép", "Bạn có thể bật lại trong cài đặt của trình duyệt.", "info");
  }
}
...
function saveSettings() {
  ...
  saveData(STORAGE_KEYS.settings, settings);
  showToast("Đã lưu thiết lập", "Lịch nhắc cố định ngày 10 hằng tháng đã được cập nhật.", "success");
}
...
function addCustomer(form) {
  ...
  showToast("Đã thêm khách hàng", `${customer.name} được đặt hạn vào ngày 10 hằng tháng.`, "success");
}
...
document.addEventListener("click", event => {
  ...
  if (customerElement && !event.target.closest("a, button, select, input, textarea, label")) {
...
document.addEventListener("keydown", event => {
  ...
  if ((event.key === "Enter" || event.key === " ") && event.target.matches("[data-customer-id]")) {
    event.preventDefault();
...
document.getElementById("customer-filters").addEventListener("click", event => {
...
document.getElementById("hero-send-reminder").addEventListener("click", () => sendReminder());
document.getElementById("save-settings").addEventListener("click", saveSettings);
document.getElementById("enable-browser-notifications").addEventListener("click", requestBrowserNotification);
...
document.getElementById("customer-detail-content").addEventListener("click", event => {
  ...
  if (button.dataset.detailAction === "paid") markCustomerPaid(button.dataset.id);
});
...
document.getElementById("add-customer-button").addEventListener("click", () => {
  ...
  document.body.style.overflow = "hidden";
  setTimeout(() => document.querySelector('#customer-form input[name="name"]').focus(), 50);
});
["close-add-modal", "cancel-add-customer"].forEach(id => document.getElementById(id).addEventListener("click", () => closeModal("add-customer-modal")));
document.getElementById("add-customer-modal").addEventListener("click", event => {
  if (event.target.id === "add-customer-modal") closeModal("add-customer-modal");
});
